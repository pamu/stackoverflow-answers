...
object Conversions {
  ...
  def transformJString(json: org.json4s.JValue)(transform: String => String): j4s.JValue = json match {
    case j4s.JString(str) => j4s.JString(transform(str))
    case j4s.JArray(fields) => j4s.JArray(fields.map(a => transformJString(a)(transform)))
    case j4s.JObject(fields) => j4s.JObject(fields.map { case (k, v) => k -> transformJString(v)(transform) })
    case others => others
  }
